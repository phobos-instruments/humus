...
// Humus: the ratio-mode pitch math, from an explicit base log2-frequency.
int32_t osc_freq_base(int32_t base_logfreq, int mode, int coarse, int fine,
                      int detune) {
  int32_t logfreq;
  if (mode == 0) {
    logfreq = base_logfreq;
...
#ifdef VERBOSE
    std::cout << op << ": " << level_scaling << " " << outlevel << std::endl;
#endif
    outlevel = outlevel << 5;
...
// Humus: same as init(), but pitch comes from base_logfreq (Q24 log2 Hz) so a
// microtonal tuning lands exactly; scalenote only feeds the scaling curves.
void Dx7Note::initLogfreq(const char patch[156], int32_t base_logfreq,
                          int scalenote, int velocity) {
  int rates[4];
  int levels[4];
  for (int op = 0; op < 6; op++) {
    int off = op * 21;
    ...
    int outlevel = patch[off + 16];
    outlevel = Env::scaleoutlevel(outlevel);
    int level_scaling = ScaleLevel(scalenote, patch[off + 8], patch[off + 9],
        patch[off + 10], patch[off + 11], patch[off + 12]);
    outlevel += level_scaling;
    outlevel = min(127, outlevel);
    outlevel = outlevel << 5;
    outlevel += ScaleVelocity(velocity, patch[off + 15]);
    outlevel = max(0, outlevel);
    int rate_scaling = ScaleRate(scalenote, patch[off + 13]);
    env_[op].init(rates, levels, outlevel, rate_scaling);
    ...
    int fine = patch[off + 19];
    int detune = patch[off + 20];
    basepitch_[op] = osc_freq_base(base_logfreq, mode, coarse, fine, detune);
    params_[op].phase = 0;
    params_[op].gain[1] = 0;
  }
  for (int i = 0; i < 4; i++) {
