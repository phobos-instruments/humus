...
public:
    ...
    bool takeLatencyChanges();

    void setAutomation(std::vector<AutoLane> lanes);
    void setMeterMap(std::vector<MeterChange> changes);
...
private:
    struct Node {
        ...
        LatencyReporting* latency = nullptr;
...
private:
    ...
    std::vector<std::vector<int>> inCords_;
    std::vector<std::vector<int>> inMidiCords_;
    const std::vector<int>& inboundCords(int node) const;
