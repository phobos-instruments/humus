...
inline constexpr const char* kControlInletClass = "PodControlIn";
inline constexpr const char* kControlOutletClass = "PodControlOut";
inline constexpr const char* kControlPortParam = "Value";
inline constexpr const char* kControlPortValue = "value";

inline bool isControlPortClass(const std::string& c) {
    return c == kControlInletClass || c == kControlOutletClass;
}
inline bool isInletClass(const std::string& c) {
    return c == kInletClass || c == kInletStereoClass || c == kMidiInletClass
        || c == kVideoInletClass || c == kControlInletClass
...
inline bool isOutletClass(const std::string& c) {
    return c == kOutletClass || c == kOutletStereoClass || c == kMidiOutletClass
        || c == kVideoOutletClass || c == kControlOutletClass
...
inline int portChannels(const std::string& c) {
    if (isMidiPortClass(c) || isVideoPortClass(c) || isControlPortClass(c)) return 0;
...
inline std::vector<std::string> controlPortRefs(const PatchDocumentModel& m, const std::string& pod,
                                                bool inletSide) {
    std::vector<std::string> out;
    for (const auto& n : ports(m, pod, inletSide))
        for (const auto& c : m.organisms)
            if (c.name == n) {
                if (isControlPortClass(c.displayClass)) out.push_back(n);
                break;
            }
    return out;
}
...
inline Surface controlSurfaceOf(const PatchDocumentModel& m, const std::string& node, int port,
                                bool isDstSide, const std::string& scope) {
    if (inScope(node, scope)) return {Surface::Self, {}, port};
    const auto pod = childPodOf(node, scope);
    if (pod.empty()) return {};
    const auto list = controlPortRefs(m, pod, isDstSide);
    const auto it = std::find(list.begin(), list.end(), node);
    if (it == list.end()) return {};
    return {Surface::Box, pod, (int) (it - list.begin())};
}
