...
bool MainComponent::keyPressed(const juce::KeyPress& k) {
    ...
    if (k == KeyPress('r', cmd, 0)) { canvas_->renameSelection(); return true; }
...
void MainComponent::resized() {
    ...
    iconIn(row2, globalDiceBtn_); iconIn(row2, keepBtn_);
    iconIn(row2, undoBtn_); iconIn(row2, redoBtn_);
    sep(row2);
    meter_.setBounds(row2.removeFromRight(72).reduced(0, 6));
    row2.removeFromRight(6);
    if (showDsp) {
        dspLabel_.setBounds(row2.removeFromRight(58));
        row2.removeFromRight(2);
    }
    masterLevel_.setBounds(row2.removeFromRight(30));
    masterCaption_.setBounds(row2.removeFromRight(30));
    row2.removeFromRight(4);
    limiterBtn_.setBounds(row2.removeFromRight(34).reduced(0, 6));
    row2.removeFromRight(4);
    grooveBtn_.setBounds(row2.removeFromRight(grooveBtn_.getButtonText().length() > 6 ? 92 : 58)
                             .reduced(0, 6));
    ...
    if (showTaps) {
        tapBtn_.setBounds(row2.removeFromLeft(38).reduced(0, 3));
        row2.removeFromLeft(3);
        beat1Btn_.setBounds(row2.removeFromLeft(24).reduced(0, 3));
        row2.removeFromLeft(3);
    }
    if (showClick) {
        metroBtn_.setBounds(row2.removeFromLeft(44).reduced(0, 3));
        row2.removeFromLeft(3);
    }
    if (showLink) linkBtn_.setBounds(row2.removeFromLeft(48).reduced(0, 3));
