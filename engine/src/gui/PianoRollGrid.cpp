...
void PianoRollEditor::paint(juce::Graphics& g) {
    ...
    if (host_.isPlaying()) {
        ...
        if (tick >= 0.0) {
            ...
            g.drawLine(x, (float) gridTop(), x, (float) gridBottom(), 1.5f);
        }
    }
...
void PianoRollEditor::paintRuler(juce::Graphics& g) {
    ...
    const int barTicks = 4 * Pattern::kTicksPerBeat;
...
void PianoRollEditor::paintGrid(juce::Graphics& g) {
    ...
    for (int t = 0; t <= dur; t += snap) {
        ...
        g.setColour(bar ? Palette::border : beat ? Palette::border.withAlpha(0.55f)
                                                 : Palette::border.withAlpha(0.25f));
