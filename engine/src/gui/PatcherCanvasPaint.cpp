...
void PatcherCanvas::paint(juce::Graphics& g) {
    ...
    auto drawCord = [&](juce::Point<float> a, juce::Point<float> b, bool midi,
        ...
        const auto base = midi ? Palette::midiCord()
                         : video ? Palette::videoCord() : Palette::cord;
        g.setColour(sel ? Palette::accent : (hov ? Palette::accent.withAlpha(0.6f) : base));
    ...
    if (drag_ == Drag::Cord) {
        ...
        auto a = cordMidi_ ? midiOutletPos(dragNode_, cordOutlet_).toFloat()
    ...
    for (auto& d : displayNodes()) {
        ...
        if (d.pod) {
            ...
            g.setColour(sel ? Palette::background.withAlpha(0.5f)
        ...
        if (!d.pod && !sel) {
            if (const auto* cm = host_.model().byName(name)) {
                ...
                if (f != Family::Utility) {
                    ...
                    juce::ColourGradient spine(fam.withAlpha(0.0f), x0, y,
                                               fam.withAlpha(0.0f), x1, y, false);
                    spine.addColour(0.18, fam.withAlpha(0.95f));
                    spine.addColour(0.82, fam.withAlpha(0.95f));
        ...
        if (off) {
            g.setColour(Palette::recordRed().withAlpha(0.8f));
        ...
        auto midiSeed = [&](juce::Point<int> p) {
            if (midiFlash) {
                g.setColour(Palette::midiCord().withAlpha(0.35f));
    ...
    if (drag_ == Drag::Cord && cordTarget_) {
        ...
        auto pp = cordMidi_ ? midiInletPos(cordTargetNode_, cordTargetPort_)
...
void PatcherCanvas::CrumbBar::paint(juce::Graphics& g) {
    ...
    g.setColour(Palette::panelLight.withAlpha(0.92f));
