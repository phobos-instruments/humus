...
void paintFaderGroove(juce::Graphics& g, juce::Rectangle<float> b,
    ...
    {
        juce::ColourGradient shadow(juce::Colours::black.withAlpha(0.40f),
...
void HumLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int w, int h,
    ...
    g.setColour(Palette::panelLight.withAlpha(0.9f));
    ...
    if (sweepTo - sweepFrom > 0.01f) {
        ...
        if (radius > 14.0f) {
            ...
            g.setColour(famBright.withAlpha(0.25f));
    ...
    if (meter >= 0.0 && knobR > 6.0f) {
        ...
        g.setColour(Palette::background.withAlpha(0.75f));
    ...
    if (knobR >= 8.0f) {
        juce::ColourGradient dim(juce::Colours::black.withAlpha(0.45f), p.x, p.y,
...

    {
        ...
        juce::ColourGradient halo(famBright.withAlpha(0.34f + 0.30f * ember), tip.x, tip.y,
                                  famBright.withAlpha(0.0f), tip.x + haloR, tip.y, true);
