...
public:
    ...
    SetValuePopup(const juce::String& title, double min, double max, double interval,
        ...
        : min_(min), max_(max), interval_(interval), unit_(unit), showPitch_(showPitch),
          isRange_(isRange), current_(v1), onApply_(std::move(onApply)) {
        ...
        auto initBox = [this](ValueBox& ed, double v) {
            ...
            ed.setInputRestrictions(12, "0123456789.-eE*/xX");
    ...
    double enteredForTest(const juce::String& typed) {
        box1_.clear();
        box1_.insertTextAtCaret(typed);
        return entered();
    }
...
private:
    ...
    double entered() const {
        const auto text = box1_.getText();
        if (isRelativeEntry(text))
            return juce::jlimit(min_, max_, applyRelativeEntry(text, current_));
        return valueFor(unit_, text, min_, max_);
    }

    void apply() {
        const double v1 = entered();
        if (isRange_) {
