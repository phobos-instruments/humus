...
public:
    SoundMapView(EngineHost& host, std::string organism, std::string paramX, std::string paramY)
        : PolledBrick(host, std::move(organism), 2),
          px_(std::move(paramX)), py_(std::move(paramY)) {
    ...
    void paint(juce::Graphics& g) override {
        ...
        if (dots_.isValid())
            g.drawImage(dots_, getLocalBounds().toFloat());
        g.setColour(Palette::border);
        ...
        const float spray = (float) host_.liveParamValue(name_, "Spray") * w;
        if (spray > 1.0f) {
        ...
        g.setColour(Palette::background.darker(0.6f).withAlpha(0.85f));
...
private:
    void poll() override {
        if (auto* cap = dynamic_cast<LiveCaptureSource*>(host_.liveOrganism(name_))) {
            ...
            if (cap->fetchCompletedTake(take))
                host_.setParamText(name_, "File" + std::to_string(take.slot), take.path);
        }
        repaint();
    }
    ...
    static juce::Colour fileColour(int file) {
        ...
        return Palette::accent.withRotatedHue(rot[file & 3]).withAlpha(0.75f);
    }
