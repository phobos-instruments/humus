...
void ClipEditorView::paint(juce::Graphics& g) {
    g.setOrigin(-getPosition());
    paintClip(g);
}

void ClipEditorView::paintCutGuide(juce::Graphics& g) {
    if (cutguide::active(ctx_, hover_)) cutguide::paint(g, cutguide::x(view_, ctx_, hover_.x), getBottom());
}

void ClipEditorView::paintClip(juce::Graphics& g) {
    ...
    g.fillRect(f);
    g.setColour(accent.withAlpha(alpha::mist));
    ...
    timelinechrome::paintFadeGrips(g, box, tracksgeo::kFadeGrip * 2, ci.fadeInTicks, ci.fadeOutTicks, accent);
    ...
    if (view_.sel.active && view_.sel.to > view_.sel.from) {
        const float x0 = beatToX(view_.sel.from), x1 = beatToX(view_.sel.to);
    ...
    paintCutGuide(g);
    timelinechrome::paintPlayhead(g, beatToX(view_.playBeat), (float) tracksgeo::rulerTop(), (float) f.getBottom(),
                                  (float) tracksgeo::kStripW, (float) getWidth(), true);
}

void ClipEditorView::paintClipWave(juce::Graphics& g, juce::Rectangle<int> f,
    ...
    const double fileRate = peaks->fileSampleRate > 0.0 ? peaks->fileSampleRate : host().sampleRate();
    const double toFile = fileRate / host().sampleRate();
    ...
    const auto& mono = window_.read(ci.audioFile, from, to);
    if (mono.empty()) return;
    const auto base = window_.from();
    ...
    g.setColour(accent.brighter(0.35f).withAlpha(alpha::heavy));
    g.strokePath(line, juce::PathStrokeType(1.0f));
}

void ClipEditorView::paintClipRibbon(juce::Graphics& g, const ClipEditor::ClipInfo& ci) {
    const auto r = ribbonBounds();
    g.setColour(Palette::panel);
    g.fillRect(r);
    g.setColour(Palette::border);
    g.drawHorizontalLine(r.getY(), (float) r.getX(), (float) r.getRight());
    const double sr = host().sampleRate() > 0.0 ? host().sampleRate() : 48000.0;
