...
inline void clipRows(std::vector<Row>& rows, TracksHost& host, const std::string& node,
    ...
    rows.push_back({tr("media-info.starts", "Starts"),
                    tr("media-info.bar", "bar") + " " + juce::String(meters.barAt(startBeat))
                        + "." + juce::String((int) std::floor(meters.beatInBar(startBeat) + 1.0e-9))
...
inline void show(TracksHost& host, const std::string& node, const ClipEditor::ClipInfo& ci) {
