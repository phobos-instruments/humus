...
public:
    ...
    void paint(juce::Graphics& g) override {
        paintBrandPanel(g, getLocalBounds(), ink::brand::ground, ink::brand::line);
        g.setColour(Palette::background);
        g.fillRect(juce::Rectangle<int>(0, kHeaderH, getWidth(), kContentH));

        if (!hero_.isEmpty()) {
            g.setColour(ink::brand::ground);
            g.fillRect(0, kHeaderH, getWidth(), (int) hero_.getBottom() + 12 - kHeaderH);
            g.setColour(ink::brand::line.withAlpha(alpha::veil));
        ...
        for (int i = 0; i < n; ++i) {
            g.setColour(i == (int) page_ ? Palette::accent
                                         : ink::brand::line.withAlpha(i < (int) page_ ? 0.55f : 0.22f));
...
private:
    ...
    class PodSketch : public juce::Component {
        ...
        static void node(juce::Graphics& g, juce::Rectangle<float> r,
            ...
            g.setColour(passThrough ? Palette::accent.withAlpha(alpha::mid) : Palette::border);
        ...
        void paint(juce::Graphics& g) override {
            ...
            g.setColour(Palette::panel.withAlpha(alpha::mid));
