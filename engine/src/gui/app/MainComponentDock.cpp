...
void MainComponent::buildDock() {
    ...
    splitV_.onDragEnd = [this] { propsPane_->setDragLayout(false); resized(); persistDock(); };
    splitH_.onDragEnd = [this] { propsPane_->setDragLayout(false); resized(); persistDock(); };
}
...
void MainComponent::queueDockRelayout() {
    if (dockRelayoutQueued_) return;
    dockRelayoutQueued_ = true;
    juce::Timer::callAfterDelay(16, [safe = juce::Component::SafePointer<MainComponent>(this)] {
        if (safe == nullptr) return;
        safe->dockRelayoutQueued_ = false;
        safe->resized();
    });
}
