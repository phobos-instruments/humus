...
#include "core/library/ExamplesTree.h"

#include "gui/video/VisualPlanBuilder.h"

#include "gui/assistant/SampleLab.h"
...
juce::PopupMenu MainComponent::getMenuForIndex(int index, const juce::String&) {
    ...
    if (index == 0) {
...
#endif
        {
            ...
            addExampleItems(examplesMenu, examples::shipped());
            m.addSubMenu(tr("menu.examples", "Examples"), examplesMenu, !exampleMenu_.empty());
        }
        m.addSeparator();
#if JUCE_MAC
        ...
        m.addItem(34, tr("menu.rename-organism", "Rename Organism... (F2)"), !canvas_->selected().empty());
        m.addSeparator();
...
#if JUCE_MAC
        m.addItem(25, tr("menu.capture-performance-mac", "Capture Performance (Cmd+R)"), true, host_.record().armed());
#else
        m.addItem(25, tr("menu.capture-performance-win", "Capture Performance (Ctrl+R)"), true, host_.record().armed());
#endif
...
void MainComponent::menuItemSelected(int id, int) {
    ...
    switch (id) {
        ...
        default:
            ...
            if (id >= kExamplesIdBase && id < kExamplesIdBase + (int) exampleMenu_.size()) {
                const auto f = exampleMenu_[(size_t) (id - kExamplesIdBase)];
                confirmDiscardThenRun([this, f] { openFileAt(f); });
            }
            break;
    }
...
void MainComponent::addExampleItems(juce::PopupMenu& into, const examples::Node& node) {
    for (const auto& c : node.children) {
        if (c.isPatch()) {
            const int id = kExamplesIdBase + (int) exampleMenu_.size();
            exampleMenu_.push_back(c.file);
            into.addItem(id, juce::String::fromUTF8(c.label.c_str()));
            continue;
        }
        juce::PopupMenu sub;
        addExampleItems(sub, c);
        into.addSubMenu(juce::String::fromUTF8(c.label.c_str()), sub);
    }
}
