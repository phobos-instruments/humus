...
inline std::vector<SoundKind> soundKinds() {
    return {{"wav", tr("bounce.wav", "WAV, 24-bit"), 288.0e3},
            ...
            {"mp3", tr("bounce.mp3", "MP3"), mp3BytesPerSecond(kMp3RateBest)}};
}
...
inline double soundBytesPerSecond(const BounceWants& w) {
    ...
    if (w.soundKind == "mp3") return mp3BytesPerSecond(w.mp3Rate);
    return soundKindOf(w.soundKind).bytesPerSecond;
}

inline juce::String mp3RateLabel(int rate) {
    const int step = juce::jlimit(0, kMp3RateSteps - 1, rate);
    juce::String out = juce::String(kMp3KbitPerSecond[step]) + " "
                       + tr("bounce.kbit-per-second", "kbit/s");
    if (step == kMp3RateBest) out += " (" + tr("bounce.best", "best") + ")";
    return out;
}
