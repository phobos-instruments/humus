...
int TracksPane::clipIndexOfId(const std::string& node, int id) const {
    if (id <= 0) return -1;
...
void TracksPane::applyRepeatFill(int endTick) {
    ...
    while ((int) repeatIds_.size() > want) {
        if (const int i = clipIndexOfId(node, repeatIds_.back()); i >= 0)
            host_.clips().remove(node, i);
        repeatIds_.pop_back();
    }
    while ((int) repeatIds_.size() < want) {
        const int at = repeatStart_ + (int) (repeatIds_.size() + 1) * repeatLen_;
        const int n = host_.clips().duplicate(node, clipIndexOfId(node, repeatSrcId_), at);
        if (n < 0) break;
        repeatIds_.push_back(host_.clips().list(node)[(size_t) n].id);
    }
    if (!repeatIds_.empty())
