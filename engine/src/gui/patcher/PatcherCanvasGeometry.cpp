...
void PatcherCanvas::portCounts(const std::string& name, int& ins, int& outs) const {
    if (isPodBox(name)) {
        ...
        auto& h = const_cast<PatcherHost&>(host_);
        ins = h.inletsOf(name);
...
void PatcherCanvas::controlPortCounts(const std::string& name, int& ins, int& outs) const {
    ins = outs = 0;
    auto& h = const_cast<PatcherHost&>(host_);
    ins = h.controlInletsOf(name);
    outs = h.controlOutletsOf(name);
    if (isPodBox(name)) return;
    if (!scope_.empty() && pods::parentOf(name) == scope_)
        if (const auto* cm = host_.model().byName(name)) {
            if (cm->displayClass == pods::kControlInletClass) ins = 0;
            else if (cm->displayClass == pods::kControlOutletClass) outs = 0;
        }
}
...
bool PatcherCanvas::hitCord(juce::Point<int> p, Edge& out) const {
    ...
    for (const auto& c : self->host_.controlCordsInScope(scope_)) {
        ...
        path.getNearestPoint(fp, nearest);
        const float d = nearest.getDistanceFrom(fp);
        if (d < 6.0f && d < best) {
            best = d;
            out = Edge{c.src, c.srcOutlet, c.dst, c.dstInlet, false, false, true};
        }
    }
...
bool PatcherCanvas::hitPort(juce::Point<int> p, std::string& node, int& port,
                            bool& isOutlet, bool& isMidi, bool& isVideo, bool& isControl) {
    isControl = false;
    for (auto& d : displayNodes()) {
        int ins, outs, mIns, mOuts, vIns, vOuts, cIns, cOuts;
        ...
        controlPortCounts(d.name, cIns, cOuts);
        for (int i = 0; i < cIns; ++i)
            if (controlInletPos(d.name, i).getDistanceFrom(p) <= kPort / 2.0f + 3.0f) {
                node = d.name; port = i; isOutlet = false; isMidi = false; isVideo = false; isControl = true; return true;
            }
        for (int o = 0; o < cOuts; ++o)
            if (controlOutletPos(d.name, o).getDistanceFrom(p) <= kPort / 2.0f + 3.0f) {
                node = d.name; port = o; isOutlet = true; isMidi = false; isVideo = false; isControl = true; return true;
            }
        if (d.pod) {
