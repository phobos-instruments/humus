...
void PatcherCanvas::showAddMenu(juce::Point<int> at, juce::Point<int> screenPos) {
    enum { kNewPod = 900001, kPodFromPatch, kAddInlet, kAddOutlet,
           ...
           kAddControlInlet, kAddControlOutlet,
           kArrange, kSearch };
    ...
    if (!scope_.empty()) {
        ...
        menu.addItem(kAddControlInlet, tr("patcher-canvas-menus.add-pod-control-inlet", "Add Pod Control Inlet"));
        menu.addItem(kAddControlOutlet, tr("patcher-canvas-menus.add-pod-control-outlet", "Add Pod Control Outlet"));
    }
    menu.addSeparator();
    ...
    menu.showMenuAsync(juce::PopupMenu::Options()
        ...
        if (result == kAddInlet || result == kAddOutlet
            ...
            || result == kAddControlInlet || result == kAddControlOutlet) {
            const char* cls = result == kAddInlet         ? pods::kInletClass
                            ...
                            : result == kAddVideoOutlet   ? pods::kVideoOutletClass
                            : result == kAddControlInlet  ? pods::kControlInletClass
                                                          : pods::kControlOutletClass;
...
void PatcherCanvas::showCordMenu(const Edge& cord, juce::Point<int> at, juce::Point<int> screenPos) {
    ...
    std::vector<std::string> classOrder;
    juce::PopupMenu menu;
    if (!cord.midi && !cord.video && !cord.control) {
        if (modernMenusEnabled()) {
    ...
    menu.showMenuAsync(juce::PopupMenu::Options()
        ...
        if (r == Delete_) {
            if (cord.control)
                host_.disconnectControl(cord.src, cord.srcOutlet, cord.dst, cord.dstInlet);
            else if (cord.midi)
