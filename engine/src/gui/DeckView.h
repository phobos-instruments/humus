...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        BeatGrid grid{juce::jmax(1.0, host_.liveParamValue(name_, "BPM")),
                      (int64_t) host_.liveParamValue(name_, "GridOffset")};

        paintWave(g, overview_, peaks, 0.0, (double) len, len, Palette::accent.withAlpha(0.55f));
        auto ovX = [&](double s) { return overview_.getX() + (float) (s / (double) len) * overview_.getWidth(); };
        g.setColour(Palette::text.withAlpha(0.18f));
        ...
        if (host_.liveParamValue(name_, "Loop") >= 0.5) {
            const double li = host_.liveParamValue(name_, "LoopIn"), lo = host_.liveParamValue(name_, "LoopOut");
            if (lo > li) { g.setColour(Palette::accent.withAlpha(0.15f));
        ...
        if (spb > 1.0) {
            ...
            for (double bk = b0; ; bk += 1.0) {
                ...
                g.setColour(down ? Palette::text.withAlpha(0.6f) : Palette::border.withAlpha(0.45f));
        ...
        const double cue = host_.liveParamValue(name_, "Cue");
        if (cue > 0 && inView(cue)) {
        ...
        for (int i = 1; i <= 8; ++i) {
            const double h = host_.liveParamValue(name_, "HotCue_" + std::to_string(i));
    ...
    void filesDropped(const juce::StringArray& files, int, int) override {
        for (auto& f : files) if (hasAudioExt(f)) { host_.setParamText(name_, "File", f.toStdString()); break; }
        repaint();
    }
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        if (e.mods.isAltDown()) {
            host_.setParam(name_, "GridOffset", (double) mainXToSample(e.x)); drag_ = Drag::None; repaint(); return;
        }
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        if (drag_ == Drag::Loop) {
            ...
            host_.setParam(name_, "LoopIn", (double) std::min(a, b));
            host_.setParam(name_, "LoopOut", (double) std::max(a, b));
            repaint(); return;
        }
        if (drag_ == Drag::Scrub) {
            const double fileSr = host_.decks().sampleRate(name_);
...
private:
    ...
    void scaleBpm(double k) {
        const double b = host_.liveParamValue(name_, "BPM") * k;
        host_.setParam(name_, "BPM", juce::jlimit(20.0, 300.0, b));
        repaint();
    }
