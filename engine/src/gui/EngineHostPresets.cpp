...
Ref PresetHost::current(const std::string& name) const {
    Ref r;
    const auto* c = host_.model_.byName(name);
...
void PresetHost::setCurrent(const std::string& name, const Ref& ref) {
    auto* c = const_cast<OrganismModel*>(host_.model_.byName(name));
    if (c == nullptr) return;
...
void PresetHost::recall(const std::string& name, const Ref& ref) {
        ...
        for (const auto& def : factoryPresetsFor(c->classRaw)) {
            ...
            for (const auto& [k, v] : def.values) {
                ...
                if (next != cls && PackRegistry::instance().classManifest(next) != nullptr) {
                    node = host_.replaceOrganism(name, next);
                    reshaped = true;
                }
                break;
            }
    ...
    auto* cn = const_cast<OrganismModel*>(host_.model_.byName(node));
    if (!cn) return;
    ...
    if (reshaped && host_.onTopologyChanged)
        juce::MessageManager::callAsync([cb = host_.onTopologyChanged] { cb(); });
}
...
Ref PresetHost::recallAdjacent(const std::string& name, int dir) {
    const auto all = presets::stack(host_.model_.byName(name));
    if (all.empty()) return {};
...
bool PresetHost::canPaste(const std::string& name) const {
    auto* c = host_.model_.byName(name);
