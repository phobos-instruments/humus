...
inline juce::String applyRecipes(EngineHost& host, const std::string& targetSampler,
    ...
    if (sampler.empty() || !host.model().byName(sampler))
        sampler = host.addOrganism("Sampler", {160, 160});
...
inline void open(EngineHost& host, std::function<void(juce::String status)> onStatus) {
    juce::StringArray samplers;
    for (const auto& cm : host.model().organisms)
        if (cm.displayClass == "Sampler") samplers.add(juce::String(cm.name));
