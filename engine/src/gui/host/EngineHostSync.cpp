...
void EngineHost::setMidiSyncMode(int mode) {
    ...
    if (mode == kSyncGenerate) {
        ...
        syncSetOutput(midiState_.outs[0].get());
...
void EngineHost::syncTransportEvent(bool starting) {
    if (syncMode_.load(std::memory_order_relaxed) != kSyncGenerate || midiState_.outs[0] == nullptr)
        return;
    if (!starting) {
        midiState_.outs[0]->sendMessageNow(juce::MidiMessage::midiStop());
        return;
    }
    const double beats = positionBeats();
    if (beats < 1.0e-6) {
        midiState_.outs[0]->sendMessageNow(juce::MidiMessage::midiStart());
    } else {
        midiState_.outs[0]->sendMessageNow(juce::MidiMessage::songPositionPointer(
            juce::jlimit(0, 16383, (int) std::lround(beats * 4.0))));
        midiState_.outs[0]->sendMessageNow(juce::MidiMessage::midiContinue());
    }
}
