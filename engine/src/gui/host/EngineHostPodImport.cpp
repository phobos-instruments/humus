...
std::string EngineHost::importPatchAsPod(const std::string& path, juce::Point<int> at,
    ...
    for (const auto& cm : doc.organisms) {
        ...
        if (classHasRole(cm.classRaw, role::kMasterOut)) {
            int outs = 2;
