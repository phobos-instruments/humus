...
struct PatternEditorSpec {
    bool valid = false;
    int laneCount = 8;
    std::string masterVolumeParam;
    std::string muteParam;
    std::string gateParam;
    std::string volumeParamPrefix;
    std::string enableParamPrefix;
    std::string fileParamPrefix;
};

class PatternEditor : public OrganismEditor, private juce::Timer {
public:
    PatternEditor(BrickHost& host, std::string organism, PatternEditorSpec spec);
