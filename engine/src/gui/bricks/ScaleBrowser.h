...
public:
    ScaleFileSlot(BrickHost& host, std::string organism, std::string param, const Bindings& bound)
        : host_(host), name_(std::move(organism)), param_(std::move(param)),
          select_(bound.assignment(bind::kSelect)) {
    ...
    void browse() {
        ScaleBrowser::show(
            ...
            [this, safe = juce::Component::SafePointer<ScaleFileSlot>(this)](const std::string& path) {
                ...
                if (!select_.first.empty()) host_.editParam(name_, select_.first, select_.second);
                refresh();
            });
    }
