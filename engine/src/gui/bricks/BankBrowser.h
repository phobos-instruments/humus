...
private:
    ...
    void browse() {
        ...
        juce::Component::SafePointer<BankFileSlot> safe(this);
        BankBrowser::show(
            browseBtn_.getScreenBounds(), className(), slot_,
            [host, node, param, safe](const std::string& ref) {
    ...
    void chooseFile() {
        ...
        juce::Logger::writeToLog("bank slot " + juce::String(name_) + "/" + juce::String(param_)
                                 + ": opening the chooser");
        ...
        chooser_->launchAsync(juce::FileBrowserComponent::openMode
                              ...
                              [this](const juce::FileChooser& fc) {
                                  ...
                                  juce::Logger::writeToLog("bank slot " + juce::String(name_) + "/"
                                                           + juce::String(param_) + ": chooser returned '"
                                                           + f.getFullPathName() + "'");
                                  if (f == juce::File{}) return;
                                  const auto cls = className();
                                  const auto ref = banks::referenceFor(f, slot_, cls);
                                  BankBrowser::remember(ref, cls);
                                  host_.setParamText(name_, param_, ref);
                                  refresh();
                              });
    }
