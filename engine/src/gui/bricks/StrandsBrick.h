...
public:
    ...
    void filesDropped(const juce::StringArray& files, int x, int) override {
        host_.setParamText(name_, filePrefix_ + std::to_string(strandAtX(x) + 1),
    ...
    StrandsView(BrickHost& host, std::string organism, const Bindings& bound)
        : PolledBrick(host, std::move(organism), 2), filePrefix_(bound(bind::kFilePrefix)),
          mutePrefix_(bound(bind::kMutePrefix)), soloPrefix_(bound(bind::kSoloPrefix)) {
        lastState_.fill(-1);
        lastLayers_.fill(-1);
        lastPhase_.fill(-1);
    ...
    void paint(juce::Graphics& g) override {
        ...
        const auto red = ink::strand::mute, amber = ink::strand::solo;
        bool anySolo = false;
        for (int i = 0; i < n; ++i)
            anySolo = anySolo || host_.liveParamValue(name_, soloPrefix_ + std::to_string(i + 1)) >= 0.5;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            const bool audible = host_.liveParamValue(name_, mutePrefix_ + k) < 0.5
                                 && (!anySolo || host_.liveParamValue(name_, soloPrefix_ + k) >= 0.5);
            ...
            if (pending && (blink_ & 4) != 0) c = c.withAlpha(alpha::muted);
...
private:
    std::string filePrefix_, mutePrefix_, soloPrefix_;
    void poll() override {
        auto* st = live<StrandStatus>();
        bool moved = false;
        ...
        for (int i = 0; i < (int) lastState_.size(); ++i) {
            ...
            const int layers = st ? st->strandLayers(i) : 0;
            const std::string k = std::to_string(i + 1);
            const int ms = (host_.liveParamValue(name_, mutePrefix_ + k) >= 0.5 ? 1 : 0)
                           + (host_.liveParamValue(name_, soloPrefix_ + k) >= 0.5 ? 2 : 0);
            const bool pending = st && st->strandPending(i);
