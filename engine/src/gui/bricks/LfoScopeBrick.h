...
public:
    LfoScopeView(BrickHost& host, std::string name, const Bindings& bound)
        : PolledBrick(host, std::move(name)), waveform_(bound(bind::kWaveform)),
          amplitude_(bound(bind::kAmplitude)), offset_(bound(bind::kOffset)) {}
    ...
    void paint(juce::Graphics& g) override {
        ...
        const int wave = (int) juce::jlimit(0.0, 5.0, host_.liveParamValue(name_, waveform_));
        const double amp = juce::jlimit(0.0, 1.0, host_.liveParamValue(name_, amplitude_));
        const double off = juce::jlimit(-1.0, 1.0, host_.liveParamValue(name_, offset_));
