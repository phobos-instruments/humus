...
public:
    FileTransport(BrickHost& host, std::string organism, const Bindings& bound)
        : host_(host), name_(std::move(organism)),
          activeParam_(bound(bind::kActive)), loopParam_(bound(bind::kLoop)),
        ...
        loop_.onClick   = [this] { host_.setParam(name_, loopParam_, isLoop() ? 0.0 : 1.0); refreshStates(); };
...
private:
    ...
    bool isActive() const { return host_.liveParamValue(name_, activeParam_) >= 0.5; }
    bool isLoop() const { return host_.liveParamValue(name_, loopParam_) >= 0.5; }
    void setActive(bool on) {
        if (on) host_.ensureAudio();
        host_.setParam(name_, activeParam_, on ? 1.0 : 0.0);
        refreshStates();
    }
    ...
    BrickHost& host_;
    std::string name_;
    std::string activeParam_, loopParam_;
