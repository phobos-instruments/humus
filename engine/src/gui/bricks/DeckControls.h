...
public:
    DeckControls(BrickHost& host, std::string organism, const Bindings& bound)
        : host_(host), name_(std::move(organism)),
          p_{bound(bind::kBpm), bound(bind::kCue), bound(bind::kGridOffset), bound(bind::kHotCuePrefix),
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            hot_[(size_t) i].onClick = [this, i] {
                if (juce::ModifierKeys::getCurrentModifiers().isShiftDown()) host_.decks().clearHotCue(name_, p_, i + 1);
                else if (host_.liveParamValue(name_, p_.hotCue(i + 1)) > 0.0) host_.decks().jumpHotCue(name_, p_, i + 1);
                else host_.decks().setHotCue(name_, p_, i + 1);
            };
        }
        ...
        setCue_.onClick = [this] { host_.decks().setCue(name_, p_); };
        cue_.onClick    = [this] { host_.decks().jumpCue(name_, p_); };
        loopTgl_.onClick = [this] { host_.decks().toggleLoop(name_, p_); };
        half_.onClick   = [this] { host_.decks().scaleBeatLoop(name_, p_, 0.5); };
        dbl_.onClick    = [this] { host_.decks().scaleBeatLoop(name_, p_, 2.0); };
        for (size_t i = 0; i < loopSizes_.size(); ++i) {
            ...
            loopBtns_[i].onClick = [this, sz] { host_.decks().setBeatLoop(name_, p_, sz); };
        }
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            rollBtns_[(size_t) i].onDown = [this, sz] { host_.decks().beginLoopRoll(name_, p_, sz); };
            rollBtns_[(size_t) i].onUp = [this] { host_.decks().endLoopRoll(name_, p_); };
        }
        startTimerHz(20);
    }
...
private:
    ...
    void timerCallback() override {
        for (int i = 0; i < 8; ++i)
            hot_[(size_t) i].setToggleState(host_.liveParamValue(name_, p_.hotCue(i + 1)) >= 0.0,
                                            juce::dontSendNotification);
        loopTgl_.setToggleState(host_.liveParamValue(name_, p_.loop) >= 0.5, juce::dontSendNotification);
    }
