...
public:
    ...
    SequenceGridBrick(BrickHost& host, std::string organism, const Bindings& bound)
        : host_(host), cn_(std::move(organism)), enablePrefix_(bound(bind::kEnablePrefix)),
          notePrefix_(bound(bind::kNotePrefix)), velocityPrefix_(bound(bind::kVelocityPrefix)) {
        ...
        for (int r = 0; r < kRows; ++r) {
            ...
            applyFineCrawl(k);
            k.setValue(host_.liveParamValue(cn_, param(velocityPrefix_, r)),
                       juce::dontSendNotification);
            k.onValueChange = [this, r] {
                host_.editParam(cn_, param(velocityPrefix_, r), vel_[(size_t) r].getValue());
            };
            addAndMakeVisible(k);
        }
        startTimerHz(30);
    ...
    void paint(juce::Graphics& g) override {
        for (int r = 0; r < kRows; ++r) {
            ...
            const bool en = host_.liveParamValue(cn_, param(enablePrefix_, r)) >= 0.5;
            ...
            g.drawText(midiNoteName((int) host_.liveParamValue(cn_, param(notePrefix_, r))),
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        for (int r = 0; r < kRows; ++r) {
            if (ledRect(r).contains(e.getPosition())) {
                const bool en = host_.liveParamValue(cn_, param(enablePrefix_, r)) >= 0.5;
                host_.editParam(cn_, param(enablePrefix_, r), en ? 0.0 : 1.0);
                repaint();
                return;
            }
            if (plateRect(r).contains(e.getPosition())) {
                auto anchor = localAreaToGlobal(plateRect(r));
                showNotePicker(host_, cn_, param(notePrefix_, r), anchor, 0, kMidiMax,
                               [this] { repaint(); });
                return;
            }
    ...
    void mouseWheelMove(const juce::MouseEvent& e,
        ...
        for (int r = 0; r < kRows; ++r)
            if (plateRect(r).contains(e.getPosition())) {
                const int note = (int) host_.liveParamValue(cn_, param(notePrefix_, r));
                const int next = juce::jlimit(0, kMidiMax, note + (wheel.deltaY > 0 ? 1 : -1));
                if (next != note) host_.editParam(cn_, param(notePrefix_, r), (double) next);
                repaint();
                return;
            }
    }
...
private:
    ...
    static std::string param(const std::string& prefix, int row) {
    ...
    BrickHost& host_;
    std::string cn_;
    std::string enablePrefix_, notePrefix_, velocityPrefix_;
