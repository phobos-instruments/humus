...
public:
    ...
    void publishFlipped(const std::string& node, int w, int h,
                        const std::uint8_t* bottomUpRgba) {
        if (w <= 0 || h <= 0 || bottomUpRgba == nullptr) return;
        const juce::ScopedLock sl(lock_);
        ...
        s.w = w;
        s.h = h;
        s.rgba.resize((size_t) w * (size_t) h * 4u);
        const size_t row = (size_t) w * 4u;
        for (int y = 0; y < h; ++y)
            std::memcpy(s.rgba.data() + row * (size_t) y,
                        bottomUpRgba + row * (size_t) (h - 1 - y), row);
        ++s.generation;
    }
    ...
    bool takeRaw(const std::string& node, unsigned& generation, int& w, int& h,
    ...
    bool take(const std::string& node, unsigned& generation, juce::Image& out) const {
        ...
        juce::Image img(juce::Image::ARGB, s.w, s.h, false);
        {
            ...
            for (int y = 0; y < s.h; ++y) {
                auto* line = (juce::PixelARGB*) bm.getLinePointer(y);
                const std::uint8_t* src = s.rgba.data() + (size_t) y * (size_t) s.w * 4u;
                for (int x = 0; x < s.w; ++x, src += 4)
                    line[x].setARGB(src[3], src[0], src[1], src[2]);
            }
        }
        out = img;
        return true;
