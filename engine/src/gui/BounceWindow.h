...
public:
    ...
    explicit BounceWindow(BounceOffer offer) : offer_(std::move(offer)) {
        ...
        for (auto* c : {&sound_, &out_, &size_, &fps_, &quality_, &range_})
    ...
    void resized() override {
        ...
        if (audio_.isEnabled()) sound_.setBounds(audioRow.withTrimmedLeft(8));
        else sound_.setBounds({});
        gap(6);
...
private:
    ...
    juce::ComboBox sound_, out_, size_, fps_, quality_, range_;
