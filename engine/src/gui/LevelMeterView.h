...
public:
    LevelMeterView(EngineHost& host, std::string name)
...
private:
    static void drawBar(juce::Graphics& g, juce::Rectangle<int> r, float level) {
        ...
        for (int i = 0; i < cells; ++i) {
            ...
            juce::Colour on = frac < 0.6f ? juce::Colour(0xff37c86a)
                            : frac < 0.85f ? juce::Colour(0xffe6c83c)
                                           : juce::Colour(0xffe1463c);
            juce::Rectangle<float> cell(r.getX() + i * cellW + 0.5f, (float) r.getY(),
                                        cellW - 1.0f, (float) r.getHeight());
            g.setColour(i < lit ? on : on.withAlpha(0.13f));
            g.fillRect(cell);
        }
    }
