...
bool TracksPane::paintTapeTiles(juce::Graphics& g, juce::Rectangle<int> inner,
...
bool TracksPane::paintReelTiles(juce::Graphics& g, juce::Rectangle<int> inner,
                                const ClipEditor::ClipInfo& ci, int depth) {
    const double bpm = host_.tempo() > 0.0 ? host_.tempo() : 120.0;
    const double spt = host_.sampleRate() * (kSecondsPerMinute / bpm) / Pattern::kTicksPerBeat;
    const int into = spt > 0.0 ? (int) std::llround((double) ci.audioOffset / spt) : 0;
    bool any = false;
    for (const auto& child : host_.clips().list(ci.audioFile)) {
...
double TracksPane::clipSecondsAtX(const ClipEditor::ClipInfo& ci, int x) const {
    const double bpm = host_.tempo() > 0.0 ? host_.tempo() : 120.0;
    ...
    const double in = (double) ci.audioOffset / host_.sampleRate();
    return in + (ci.audioReverse ? len - rel : rel);
}

void TracksPane::paintFilmstrip(juce::Graphics& g, juce::Rectangle<int> b,
