...
// name: this pack's static lib links into the app, the dyn pack dylib AND the
...
void handsCaptureIMP(id self, SEL, AVCaptureOutput*, CMSampleBufferRef sampleBuffer,
                     AVCaptureConnection*) {
    Ivar iv = class_getInstanceVariable(object_getClass(self), "humOnFrame");
    if (iv == nullptr) return;
    auto* fn = (std::function<void(void*)>*)
        *(void**) ((char*) (__bridge void*) self + ivar_getOffset(iv));
    if (fn != nullptr && *fn)
        if (CVImageBufferRef pb = CMSampleBufferGetImageBuffer(sampleBuffer))
            (*fn)((void*) pb);
}

Class handsDelegateClass() {
    static Class cls = [] {
        char name[64];
        std::snprintf(name, sizeof(name), "HumHandsCaptureDelegate_%p",
                      (void*) &handsCaptureIMP);   // unique per loaded image
        Class c = objc_allocateClassPair([NSObject class], name, 0);
        class_addIvar(c, "humOnFrame", sizeof(void*), (uint8_t) std::log2(sizeof(void*)),
                      @encode(void*));
        class_addMethod(c, @selector(captureOutput:didOutputSampleBuffer:fromConnection:),
                        (IMP) handsCaptureIMP, "v@:@@@");
...
id makeHandsDelegate(std::function<void(void*)>* fn) {
    id obj = [[handsDelegateClass() alloc] init];
...
// The device the user picked in the "video-inputs" combo, matched by
// localizedName - the same string juce::CameraDevice::getAvailableDevices()
// lists (and the same discovery set: built-in + external/Continuity cameras).
// No match (unplugged) or an empty name falls back to the system default.
AVCaptureDevice* handsDeviceNamed(const std::string& name) {
    if (!name.empty()) {
...
class MacHandsCamera final : public HandsNativeCamera {
public:
    bool start(const std::string& deviceName, std::function<void(void*)> onFrame) {
        onFrame_ = std::move(onFrame);
        delegate_ = makeHandsDelegate(&onFrame_);
        session_ = [[AVCaptureSession alloc] init];
        AVCaptureDevice* dev = handsDeviceNamed(deviceName);
        ...
        // 720p when the device offers it - Vision gets more pixels per joint
        // and the preview stops looking pixelated; 640x480 is the floor.
        ...
        out.alwaysDiscardsLateVideoFrames = YES;   // tracking wants fresh, not complete
        queue_ = dispatch_queue_create("hum.hands.camera", DISPATCH_QUEUE_SERIAL);
...
private:
    ...
    std::function<void(void*)> onFrame_;
};

}  // namespace

std::unique_ptr<HandsNativeCamera> openHandsCamera(const std::string& deviceName,
                                                   std::function<void(void*)> onFrame) {
    const AVAuthorizationStatus st =
        [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    if (st == AVAuthorizationStatusNotDetermined) {
        // Ask once; the lifecycle poll retries after the user answers.
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo
                                 completionHandler:^(BOOL) {}];
        return nullptr;
    }
    if (st != AVAuthorizationStatusAuthorized) return nullptr;
    auto cam = std::make_unique<MacHandsCamera>();
