...
inline Palm decodeOne(const float* row, float rawScore, const Anchor& a, const Config& c) {
    Palm p;
...
inline float iou(const Palm& a, const Palm& b) {
...
inline std::vector<Palm> suppress(std::vector<Palm> in, const Config& c, int maxOut) {
    std::sort(in.begin(), in.end(),
              [](const Palm& a, const Palm& b) { return a.score > b.score; });
    std::vector<Palm> out;
    for (const auto& p : in) {
...
inline std::vector<Palm> detections(const float* boxes, const float* scores,
                                    const std::vector<Anchor>& anch, const Config& c,
                                    int maxOut) {
    std::vector<Palm> hits;
    for (size_t i = 0; i < anch.size(); ++i) {
        const Palm p = decodeOne(boxes + i * 18, scores[i], anch[i], c);
...
inline Crop cropFor(const Palm& p, float scale = 2.6f, float shift = 0.0f) {
